...
fitness <- function(x_vec) {
  ...
  # cat("DEBUG - post_draws structure:\n")
  # print(str(post_draws))
  # cat("DEBUG - pred_vec length:", length(pred_vec), "\n")
  # cat("DEBUG - pred_vec range:", range(pred_vec), "\n")
  # cat("DEBUG - pred_vec unique values:", length(unique(pred_vec)), "\n")
...
for (iter in 1:n_iter) {
  ...
  gensa_res <- GenSA(
    par = NULL,
    fn = function(x) -fitness(x),  # Minimize negative
    lower = rep(0, p),
    upper = rep(1, p),
    control = list(
      max.call = 1200,
  ...
  x_next <- round(gensa_res$par)
  # CE_res <- CEoptim(
  #   f = function(x) fitness(x),  # Minimize negative fitness
  #   maximize = TRUE, 
  #   continuous = list(),           # Empty list for no continuous variables
  #   discrete = list(
  #     categories = rep(2, p),      # Each variable has 2 categories (0 or 1)
  #     probs = rep(list(c(0.5, 0.5)), p)  # Initial probabilities
  #   ),
  #   N = 60,           # Population size
  #   rho = 0.1,        # Elite fraction
  #   iterThr = 20,     # Max iterations
  #   noImproveThr = 10 # Stopping criterion
  # )
  # 
  # x_next <- CE_res$optimizer$discrete
  cat(sprintf("Iteration %02d — SA proposed: %s\n", iter, paste(x_next, collapse = "")))
...
# -------------------------
# OPTIMAL SOLUTION using Integer Programming
# -------------------------
library(lpSolve)

cat("\n=== FINDING OPTIMAL SOLUTION (IP) ===\n")
...
x_optimal <- IP_result$solution
optimal_value <- IP_result$objval

cat("OPTIMAL solution (Integer Programming):\n")
cat("  x_opt:", paste(x_optimal, collapse = ""), "\n")
cat("  f_opt:", optimal_value, "\n")
cat("  Weight:", sum(weights * x_optimal), "/", W, "\n\n")

# -------------------------
# Compare BART-BO to optimal
# -------------------------
if (!is.null(f_best)) {
  gap_vs_optimal <- (optimal_value - f_best) / optimal_value * 100
  cat(sprintf("BART-BO optimality gap: %.2f%%\n", gap_vs_optimal))
  cat(sprintf("BART-BO found: %d / %d (%.1f%% of optimal)\n", 
              f_best, optimal_value, 100*f_best/optimal_value))
}
